...
void main() {
  group('Cache Persistence Integration', () {
    ...
    test('set FireRisk; create a new FireRiskCacheImpl with a fresh SharedPreferences (same mock storage map); get returns some(); freshness==cached', () async {
      ...
      final setResult = await cache1.set(lat: 55.9533, lon: -3.1883, data: testFireRisk);
    ...
    test('cached data survives within same SharedPreferences instance', () async {
      ...
      await cache1.set(lat: 55.8642, lon: -4.2518, data: testFireRisk); // Glasgow
    ...
    test('corrupted cache data handled gracefully on persistence reload', () async {
      final mockData = <String, Object>{
        // Store valid entry
        'cache_entry_gcvwr': '{"version":"1.0","timestamp":${DateTime.utc(2025, 10, 4, 12, 0, 0).millisecondsSinceEpoch},"geohash":"gcvwr","data":{"level":"high","fwi":75.5,"source":"effis","observedAt":${DateTime.utc(2025, 10, 4, 12, 0, 0).millisecondsSinceEpoch},"freshness":"live"}}',
        // Store corrupted entry
        'cache_entry_gcpue': 'invalid json',
        // Valid metadata
        'cache_metadata': '{"totalEntries":2,"lastCleanup":${DateTime.utc(2025, 10, 4, 12, 0, 0).millisecondsSinceEpoch},"accessLog":{}}'
      };
