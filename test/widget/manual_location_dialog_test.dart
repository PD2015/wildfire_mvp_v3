...
void main() {
  group('ManualLocationDialog Widget Tests', () {
    ...
    testWidgets('dialog displays with correct title and input fields', (tester) async {
      // Arrange & Act
    ...
    testWidgets('accepts valid coordinate input with proper formatting', (tester) async {
      // Arrange
      await tester.pumpWidget(
        MaterialApp(
          home: Scaffold(
      ...
      await tester.enterText(find.byKey(const Key('latitude_field')), '55.9533');
      ...
      expect(saveButton.onPressed, isNotNull, reason: 'Save button should be enabled for valid coordinates');
      
      // No error message should be displayed
      expect(find.textContaining('must be between'), findsNothing);
    });

    testWidgets('rejects invalid coordinate ranges with clear error messages', (tester) async {
      // Arrange
      await tester.pumpWidget(
        MaterialApp(
          home: Scaffold(
      ...
      final saveButton = tester.widget<ElevatedButton>(find.byKey(const Key('save_button')));
      expect(saveButton.onPressed, isNull, reason: 'Save button should be disabled for invalid coordinates');
    });
    ...
    testWidgets('keyboard type is configured correctly for numeric input', (tester) async {
      ...
      expect(latField.keyboardType, equals(const TextInputType.numberWithOptions(signed: true, decimal: true)));
      expect(lonField.keyboardType, equals(const TextInputType.numberWithOptions(signed: true, decimal: true)));
    });
    ...
    testWidgets('input formatters are configured correctly', (tester) async {
      ...
      final formatter = latField.inputFormatters!.first as FilteringTextInputFormatter;
      expect(formatter.allow, isTrue);
    });

    group('Touch Target Size Validation (Gate C3)', () {
      testWidgets('all interactive elements meet 44dp minimum size requirement', (tester) async {
        // Arrange
        await tester.pumpWidget(
          MaterialApp(
            home: Scaffold(
        ...
        final cancelButtonSize = tester.getSize(find.byKey(const Key('cancel_button')));
        final saveButtonSize = tester.getSize(find.byKey(const Key('save_button')));
        
        const minimumSize = 44.0;
        expect(cancelButtonSize.height, greaterThanOrEqualTo(minimumSize), 
          reason: 'Cancel button height should be ≥44dp');
        expect(saveButtonSize.height, greaterThanOrEqualTo(minimumSize), 
          reason: 'Save button height should be ≥44dp');
        
        // Text fields should also be adequately sized for touch
        final latFieldSize = tester.getSize(find.byKey(const Key('latitude_field')));
        final lonFieldSize = tester.getSize(find.byKey(const Key('longitude_field')));
        
        expect(latFieldSize.height, greaterThanOrEqualTo(minimumSize), 
          reason: 'Latitude field should be ≥44dp for touch accessibility');
        expect(lonFieldSize.height, greaterThanOrEqualTo(minimumSize), 
          reason: 'Longitude field should be ≥44dp for touch accessibility');
      });
    });

    group('Accessibility Compliance (Gate C3)', () {
      testWidgets('proper semantic labels are present for screen readers', (tester) async {
        // Arrange
        await tester.pumpWidget(
          MaterialApp(
            home: Scaffold(
      ...
      testWidgets('semantic labels are meaningful and not generic', (tester) async {
        ...
        final latFieldSemantics = tester.getSemantics(find.byKey(const Key('latitude_field')));
        final lonFieldSemantics = tester.getSemantics(find.byKey(const Key('longitude_field')));
    ...
    group('Button Behavior', () {
      testWidgets('cancel button closes dialog without returning data', (tester) async {
        ...
        // Assert
        expect(find.byType(Dialog), findsNothing, reason: 'Dialog should be closed');
        expect(result, isNull, reason: 'Cancel should return null');
      });

      testWidgets('save button returns valid LatLng when coordinates are valid', (tester) async {
        // Arrange
        LatLng? result;
        await tester.pumpWidget(
          MaterialApp(
      ...
      testWidgets('save button is disabled when coordinates are invalid', (tester) async {
        // Arrange
        await tester.pumpWidget(
          MaterialApp(
            home: Scaffold(
        ...
        // Assert
        final saveButton = tester.widget<ElevatedButton>(find.byKey(const Key('save_button')));
        expect(saveButton.onPressed, isNull, reason: 'Save button should be disabled for invalid input');
      });
    });

    group('Real-time Validation', () {
      testWidgets('validation updates in real-time as user types', (tester) async {
        // Arrange
        await tester.pumpWidget(
          MaterialApp(
            home: Scaffold(
    ...
    group('Edge Cases', () {
      testWidgets('handles boundary coordinate values correctly', (tester) async {
        // Arrange
        await tester.pumpWidget(
          MaterialApp(
            home: Scaffold(
        ...
        await tester.enterText(find.byKey(const Key('longitude_field')), '180.0');
        await tester.pumpAndSettle();
        ...
        final saveButton = tester.widget<ElevatedButton>(find.byKey(const Key('save_button')));
        expect(saveButton.onPressed, isNotNull, reason: 'Boundary values should be valid');
      });
      ...
      testWidgets('handles negative coordinates correctly', (tester) async {
        ...
        await tester.enterText(find.byKey(const Key('latitude_field')), '-45.5');
        await tester.enterText(find.byKey(const Key('longitude_field')), '-122.3');
        await tester.pumpAndSettle();
        ...
        final saveButton = tester.widget<ElevatedButton>(find.byKey(const Key('save_button')));
        expect(saveButton.onPressed, isNotNull, reason: 'Negative coordinates should be valid');
      });
      ...
      testWidgets('handles empty fields gracefully', (tester) async {
        ...
        final saveButton = tester.widget<ElevatedButton>(find.byKey(const Key('save_button')));
        expect(saveButton.onPressed, isNull, reason: 'Empty fields should disable save button');
      });
    });
  });
}