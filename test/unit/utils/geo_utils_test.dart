...
void main() {
  group('GeographicUtils', () {
    group('geohash', () {
      test(
          'generates consistent 5-character alphanumeric hash for same coordinates',
          () {
        const lat = 55.9533;
        const lon = -3.1883;
      ...
      test('handles precision edge cases', () {
        ...
        final highPrecision =
            GeographicUtils.geohash(55.953252123, -3.188267456);
    ...
    group('isInScotland', () {
      test('returns true for major Scottish cities', () {
        ...
        expect(GeographicUtils.isInScotland(55.9533, -3.1883),
            isTrue); // Edinburgh
        expect(
            GeographicUtils.isInScotland(55.8642, -4.2518), isTrue); // Glasgow
        expect(
            GeographicUtils.isInScotland(57.1497, -2.0943), isTrue); // Aberdeen
        expect(
            GeographicUtils.isInScotland(56.4620, -2.9707), isTrue); // Dundee
        expect(
            GeographicUtils.isInScotland(56.1165, -3.9369), isTrue); // Stirling
      });

      test('returns false for locations outside Scotland', () {
        // Other UK locations
        expect(GeographicUtils.isInScotland(53.4808, -2.2426),
            isFalse); // Manchester
        expect(
            GeographicUtils.isInScotland(51.5074, -0.1278), isFalse); // London
        expect(GeographicUtils.isInScotland(53.9600, -1.0873), isFalse); // York
        expect(GeographicUtils.isInScotland(52.2053, 0.1218),
            isFalse); // Cambridge

        // International locations
        expect(GeographicUtils.isInScotland(40.7128, -74.0060),
            isFalse); // New York
        expect(GeographicUtils.isInScotland(48.8566, 2.3522), isFalse); // Paris
        expect(
            GeographicUtils.isInScotland(35.6762, 139.6503), isFalse); // Tokyo
      });
      ...
      test('handles boundary cases correctly with documented bounds', () {
        ...
        expect(
            GeographicUtils.isInScotland(54.59, -4.0), isFalse); // Just below
        expect(
            GeographicUtils.isInScotland(60.91, -4.0), isFalse); // Just above
      ...
      test('excludes areas clearly outside Scotland box', () {
        ...
        expect(GeographicUtils.isInScotland(54.1566, -4.4811),
            isFalse); // Douglas - below 54.6°N

        // Areas clearly east of Scotland
        expect(
            GeographicUtils.isInScotland(55.0, 2.0), isFalse); // East of 1.0°E

        // Areas clearly west of Scotland
        expect(GeographicUtils.isInScotland(55.0, -10.0),
            isFalse); // West of -9.0°W
    ...
    group('logRedact', () {
      test('redacts coordinate pairs to 2 decimal places for privacy', () {
        expect(GeographicUtils.logRedact(55.953252, -3.188267),
            equals('55.95,-3.19'));
        expect(GeographicUtils.logRedact(0.123456, -74.006012),
            equals('0.12,-74.01'));
        expect(GeographicUtils.logRedact(-89.123, 179.456),
            equals('-89.12,179.46')); // Valid coords
      });
      ...
      test('handles edge cases: zeros and boundaries', () {
        ...
        expect(
            GeographicUtils.logRedact(-90.0, -180.0), equals('-90.00,-180.00'));
      });
      ...
      test('maintains negative signs correctly', () {
        ...
        expect(GeographicUtils.logRedact(1.5, -2.7), equals('1.50,-2.70'));
        expect(GeographicUtils.logRedact(-89.999, -0.1),
            equals('-90.00,-0.10')); // Valid coords
      });

      test('consistent rounding behavior', () {
        // Test standard rounding (0.5 rounds up)
        expect(GeographicUtils.logRedact(1.234, 2.235),
            equals('1.23,2.24')); // 0.5 rounds up
        expect(GeographicUtils.logRedact(1.236, -1.235), equals('1.24,-1.24'));
      });

      test('returns INVALID_COORDS for invalid input', () {
        expect(GeographicUtils.logRedact(double.nan, 0.0),
            equals('INVALID_COORDS'));
        expect(GeographicUtils.logRedact(0.0, double.infinity),
            equals('INVALID_COORDS'));
        expect(GeographicUtils.logRedact(91.0, 0.0),
            equals('INVALID_COORDS')); // Out of range
        expect(GeographicUtils.logRedact(0.0, 181.0),
            equals('INVALID_COORDS')); // Out of range
      });

      test(
          'privacy compliance: actual coordinate pairs should be unrecoverable',
          () {
    ...
    group('Coordinate Validation', () {
      ...
      test('handles special floating point values', () {
        ...
        expect(
            () => GeographicUtils.logRedact(double.nan, 0.0), returnsNormally);
        expect(() => GeographicUtils.logRedact(0.0, double.infinity),
            returnsNormally);
        expect(() => GeographicUtils.logRedact(double.negativeInfinity, 0.0),
            returnsNormally);
      });
    });
    ...
    group('Geographic Precision and Accuracy', () {
      test('geohash provides appropriate spatial resolution', () {
        ...
        final nearbyHash =
            GeographicUtils.geohash(baseLat + 0.001, baseLon + 0.001);
        expect(baseHash.substring(0, 3),
            equals(nearbyHash.substring(0, 3))); // Share 3-char prefix
        ...
        expect(
            baseHash, isNot(equals(farHash))); // Completely different locations
      });
      ...
      test('Scotland boundary detection has reasonable buffer', () {
        ...
        expect(
            GeographicUtils.isInScotland(54.65, -2.5), isTrue); // Near Gretna
        expect(GeographicUtils.isInScotland(60.85, -1.2), isTrue); // Shetland

        // Test just outside Scotland
        expect(GeographicUtils.isInScotland(54.55, -2.5),
            isFalse); // Just south of border
      });
      ...
      test('coordinate redaction maintains geographical regions', () {
        ...
        final scottishRedacted =
            GeographicUtils.logRedact(55.953252, -3.188267);
