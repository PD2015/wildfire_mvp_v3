  ...
  Either<ApiError, List<BurntArea>> _parseResponse(String body) {
    try {
      ...
      if (json is Map && json.containsKey('features')) {
        ...
        developer.log(
          'EffisBurntAreaService: Unexpected response format',
          name: 'EffisBurntAreaService',
        );
        return const Right([]);
      }
      ...
      for (final feature in features) {
        try {
          var burntArea = BurntArea.fromJson(feature as Map<String, dynamic>);
          ...
          if (PolygonSimplifier.wouldSimplify(burntArea.boundaryPoints)) {
            ...
            burntArea = burntArea.copyWithSimplified(
      ...
      developer.log(
        'EffisBurntAreaService: Parsed ${burntAreas.length} burnt areas',
        name: 'EffisBurntAreaService',
      );
