...
void main() async {
  ...
  viewportResult.fold(
    ...
    (response) {
      ...
      debugPrint(
          '   Response time: ${stopwatch.elapsedMilliseconds}ms (simulated: ${response.responseTimeMs}ms)');
      ...
      if (response.hasIncidents) {
        ...
        for (int i = 0; i < response.incidents.length && i < 5; i++) {
          ...
          debugPrint(
              '      Confidence: ${incident.confidence?.toStringAsFixed(1)}%');
          debugPrint('      FRP: ${incident.frp?.toStringAsFixed(1)} MW');
          debugPrint('      Sensor: ${incident.sensorSource}');
          debugPrint(
              '      Detected: ${incident.detectedAt.toIso8601String()}');
        }

        if (response.incidents.length > 5) {
          debugPrint(
              '   ... and ${response.incidents.length - 5} more incidents');
        }
      }
    },
  );
  ...
  final invalidResult =
      await service.getIncidentById(incidentId: 'nonexistent_fire');

  invalidResult.fold(
    (error) => debugPrint('✅ Expected error for invalid ID: ${error.reason}'),
    (incident) =>
        debugPrint('❌ Should not find nonexistent incident: ${incident.id}'),
  );
  ...
  final oceanResult =
      await service.getIncidentsForViewport(bounds: oceanBounds);

  oceanResult.fold(
    (error) => debugPrint('❌ Ocean query failed: $error'),
    (response) {
      debugPrint(
          '✅ Ocean area: ${response.incidents.length} incidents (expected: 0)');
  ...
  highConfidenceResult.fold(
    ...
    (response) {
      debugPrint(
          '✅ High confidence (≥90%): ${response.incidents.length} incidents');
      ...
      if (response.hasIncidents) {
        ...
        debugPrint(
            '   Average confidence: ${avgConfidence.toStringAsFixed(1)}%');

        final minConfidence = response.incidents
            .map((i) => i.confidence ?? 0)
            .reduce((a, b) => a < b ? a : b);
        debugPrint(
            '   Minimum confidence: ${minConfidence.toStringAsFixed(1)}%');
      }
    },
  );
  ...
  debugPrint(
      '   Reliability: ${(successCount / results.length * 100).toStringAsFixed(1)}%');

  debugPrint('\n🎉 All ActiveFiresService tests completed!');
  debugPrint(
      '📋 Task 7: "Implement ActiveFiresService interface" - COMPLETE ✅');
...
Future<bool> _performanceTest(
    MockActiveFiresService service, int testId) async {
  try {
